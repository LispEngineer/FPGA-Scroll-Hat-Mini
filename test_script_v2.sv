...
module test_script_v2 #(
  parameter SEND_MAX = 5,
  parameter SEND_SZ = $clog2(SEND_MAX + 1),
  parameter READ_MAX = 2,
  parameter READ_SZ = $clog2(READ_MAX + 1)
) (
  input logic clk,
  input logic rst,
  ...
  output logic [SEND_SZ-1:0] i2c_send_count,
  output logic         [7:0] i2c_send_data [SEND_MAX],
  output logic [READ_SZ-1:0] i2c_read_count,
  // We don't have to read the data, we don't check it
  // output logic         [7:0] i2c_read_data [READ_MAX],
