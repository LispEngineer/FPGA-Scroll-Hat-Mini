...
module i2c_controller_v2 #(
  ...
  input  logic [READ_SZ-1:0] read_count,
  output logic         [7:0] read_data [READ_MAX],
...
// Our state machine       
state_t state = S_RESET; // This is bad, but I'm doing it anyway
logic [1:0] step;      // 0-3 is the part of the clock cycle we're in
...
logic [SEND_SZ-1:0] k_send_count;
logic         [7:0] k_send_data [SEND_MAX];
logic [READ_SZ-1:0] k_read_count;
logic [READ_SZ-1:0] read_pos;
logic [READ_SZ-1:0] last_read_pos;

localparam SEND_MAX_p1 = SEND_MAX + 1;
localparam SEND_SZ_p1 = $clog2(SEND_MAX_p1 + 1);

// We actually send this data, which includes the address.
logic            [7:0] actual_send_data [SEND_MAX_p1];
logic [SEND_SZ_p1-1:0] send_pos;
logic [SEND_SZ_p1-1:0] last_send_pos;

// We send our data as
always_comb begin: setup_actual_send_data
  actual_send_data[0] = k_address;
  for (int i = 0; i < SEND_MAX; i++)
    actual_send_data[i + 1] = k_send_data[i];
end: setup_actual_send_data
    ...
    case (state)
      ...
      S_IDLE: begin ////////////////////////////////////////////////////////
        if (activate) begin
          ...
          k_address[0] <= read;
          k_send_count <= send_count + 1'd1; // +1 for the address
          k_send_data <= send_data;
          k_read_count <= read_count;

          send_pos <= '0;
          read_pos <= '0;
          last_send_pos <= send_count;
          last_read_pos <= read_count > 0 ? read_count - 1'd1 : '0;
      ...
      S_SEND: begin  ////////////////////////////////////////////////////
        ...
        if (byte_step == 4'd8) begin
          ...
          case (step) // =================================
            0: begin
              ...
              sda_e <= 1; 
              // What bit do we send?
              sda_o <= actual_send_data[send_pos][byte_idx];
            end
        ...
        if (byte_step == 4'd8 && step == 2'd3) begin
          ...
          if (!next_got_ack) begin
          ...
          end else if (send_pos == last_send_pos) begin
            // We have sent our last byte
            ...
            // We have more bytes to write
            send_pos <= send_pos + 1'd1;
            state <= S_SEND; // Should be unnecessary
          end
      ...
      S_READ: begin  ////////////////////////////////////////////////////
        // HANDLE CURRENT BIT READING (or sending ACK/NAK)
        ...
        if (byte_step == 4'd8) begin
          ...
          case (step) // =================================
            ...
            0: begin
              ...
              if (read_pos == last_read_pos) begin
                // NAK if this is the last byte we just read, cause we're done reading.
        ...
        if (byte_step == 4'd8 && step == 2'd3) begin
          ...
          if (read_pos == last_read_pos) begin
            // We're READING, and read the last byte, so we're done
            state <= S_STOP;
            success <= 1'b1;
